...
def create_model(captcha_length, captcha_num_symbols, input_shape, model_depth=5, module_size=2):
    ...
    for i, module_length in enumerate([module_size] * model_depth):       #Create layers in model
        for j in range(module_length):
            x = keras.layers.Conv2D(32*2**min(i, 3), kernel_size=3, strides=(1,1), padding='same', kernel_initializer='he_uniform')(x)      
            #Param1 - no of filters, strides - no of pixels scanned at a time, 
            x = keras.layers.Activation('relu')(x)
            x = keras.layers.BatchNormalization()(x)      #Experiment suggests better performance when Normalised after activation
...
def main():
    ...
    # with tf.device('/device:XLA_CPU:0'):
        ...
        model.compile(loss='categorical_crossentropy', optimizer=keras.optimizers.Adam(1e-3, amsgrad=True), metrics=['accuracy'])
        model.summary()
